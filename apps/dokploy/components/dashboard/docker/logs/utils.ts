...
// Removed ansiToTailwind mapping as it's replaced by fancy-ansi's built-in styling system
// fancy-ansi handles ANSI color conversion and styling through CSS variables
// The output HTML automatically works with Next.js dark mode because it uses native
// HTML elements and colors that respect the parent theme context
...
// REMOVED: Original parseAnsi implementation replaced with fancy-ansi library
// See parseWithFancyAnsi function below for the new implementation that uses fancy-ansi
// This change improves ANSI parsing reliability and maintainability
...
/**
 * Converts ANSI-encoded text to HTML using fancy-ansi library
 * @param text The text containing ANSI escape sequences
 * @returns HTML string with appropriate styling
 */
export function parseWithFancyAnsi(text: string): string {
	return fancyAnsi.toHtml(text);
}
